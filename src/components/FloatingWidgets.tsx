        ...
        <div class="bg-white p-5 sm:p-6 rounded-3xl border border-slate-100 shadow-xl space-y-4">
          <div class="grid grid-cols-2 gap-3 md:grid-cols-4 md:gap-4">
            <div>
              ...
              <select disabled={!!forceDistrict} value={khuVuc} onChange={(e) => setKhuVuc(e.target.value)} class="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-3 text-sm font-semibold focus:outline-none focus:border-amber-500 text-slate-700">
            ...
            <div>
              ...
              <select value={loaiHinh} onChange={(e) => setLoaiHinh(e.target.value)} class="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-3 text-sm font-semibold focus:outline-none focus:border-amber-500 text-slate-700">
            ...
            <div>
              ...
              <select value={khoangGia} onChange={(e) => setKhoangGia(e.target.value)} class="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-3 text-sm font-semibold focus:outline-none focus:border-amber-500 text-slate-700">
            ...
            <div>
              ...
              <select value={huong} onChange={(e) => setHuong(e.target.value)} class="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-3 text-sm font-semibold focus:outline-none focus:border-amber-500 text-slate-700">
          ...
          <div class="flex flex-wrap gap-2 pt-3 border-t border-slate-100 items-center">
            <button onClick={() => setSelectedTag("all")} class={`text-xs font-bold px-4 py-2 rounded-xl ${selectedTag === "all" ? "bg-slate-900 text-white" : "bg-white border text-slate-600"}`}>Tất Cả</button>
            <button onClick={() => setSelectedTag("mattien")} class={`text-xs font-bold px-4 py-2 rounded-xl ${selectedTag === "mattien" ? "bg-slate-900 text-white" : "bg-white border text-slate-600"}`}>Mặt Tiền Kinh Doanh</button>
            <button onClick={() => setSelectedTag("chinhchu")} class={`text-xs font-bold px-4 py-2 rounded-xl ${selectedTag === "chinhchu" ? "bg-slate-900 text-white" : "bg-white border text-slate-600"}`}>Hàng Chính Chủ</button>
          </div>
        </div>
      </section>

      <main id="listing-section" class="max-w-7xl mx-auto w-full px-4 mt-16 mb-20">
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-7">
          {currentItems.map((item) => {
            ...
            return (
              <article key={item.id} onClick={() => { setSelectedItem(item); setIsOpenDetail(true); }} class="bg-white rounded-3xl overflow-hidden border border-slate-100 shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col group cursor-pointer transform hover:-translate-y-1">
                <div class="relative aspect-[4/3] overflow-hidden bg-slate-100">
                  <Image src={thumbnail} alt={item.tieude} fill class="object-cover group-hover:scale-105 transition-transform duration-700" />
                  <span class={`absolute top-3 left-3 ${item.tagColor || 'bg-slate-900'} text-white font-bold text-[10px] uppercase px-2.5 py-1 rounded-lg`}>{item.tag || 'Bán Đất'}</span>
                  {item.huong && <span class="absolute top-3 right-3 bg-white/95 text-slate-800 font-extrabold text-[10px] px-2.5 py-1 rounded-lg flex items-center gap-1"><Compass class="w-3 h-3 text-amber-500" />{item.huong}</span>}
                  <span class="absolute bottom-3 left-3 bg-black/60 text-white text-[10px] font-semibold px-2.5 py-1 rounded-lg flex items-center gap-1"><Clock class="w-3 h-3 text-amber-400" /> {formatTimeAgo(item.ngayDang)}</span>
                  <span class="absolute bottom-3 right-3 bg-slate-900/90 text-white font-extrabold text-sm px-3 py-1 rounded-xl shadow-md">{item.gia}</span>
                </div>
                <div class="p-5 flex-1 flex flex-col justify-between">
                  <div>
                    <div class="flex items-center gap-1 text-slate-400 text-xs font-semibold uppercase tracking-wider mb-2"><MapPin class="w-3.5 h-3.5 text-amber-500" /><span class="truncate">{item.khuVucFull}</span></div>
                  ...
                  <div class="mt-4 pt-4 border-t border-slate-50 flex items-center justify-between text-slate-500 text-sm font-medium">
                    <div class="grid grid-cols-2 gap-x-4 gap-y-1 text-xs text-slate-400"><span><Square class="w-3.5 h-3.5 inline mr-0.5" /> {item.dienTich}</span><span><Bed class="w-3.5 h-3.5 inline mr-0.5" /> {item.phongNgu || 'Đất ở'}</span></div>
                    <span class="text-amber-500 font-bold flex items-center gap-0.5 text-xs uppercase tracking-wider">Chi tiết <ChevronRight class="w-3 h-3" /></span>
                  </div>
                </div>
              </article>
            );
        ...
        {totalPages > 1 && (
          <div class="flex justify-center items-center gap-2 mt-10">
            {Array.from({ length: totalPages }, (_, idx) => (
              <button key={idx} onClick={() => setCurrentPage(idx + 1)} class={`w-9 h-9 rounded-xl text-sm transition-all font-bold ${currentPage === idx + 1 ? "bg-amber-500 text-slate-900 scale-105" : "bg-white border text-slate-600"}`}>{idx + 1}</button>
            ))}
          </div>
        )}
      </main>
