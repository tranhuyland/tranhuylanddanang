...
export default function ListingSection({ allBdsItems = [], forceDistrict }: ListingSectionProps) {
  ...
  // State lưu trữ tọa độ khi khách hàng vuốt
      ...
      {isDrawerOpen && (
        <div className="fixed inset-0 z-50 md:hidden flex flex-col justify-end">
          ...
          <div className="relative bg-white rounded-t-[2.5rem] shadow-2xl h-[85vh] flex flex-col z-10 overflow-hidden animate-in slide-in-from-bottom-8 duration-400 ease-out">
            ...
            {/* VÙNG HEADER CÓ THỂ VUỐT XUỐNG ĐỂ TẮT (SWIPE DOWN TO CLOSE) */}
            <div 
              className="w-full bg-white/95 backdrop-blur-md z-20 shrink-0"
              onTouchStart={(e) => setTouchStartY(e.touches[0].clientY)}
              onTouchEnd={(e) => {
                const touchEndY = e.changedTouches[0].clientY;
                // Nếu khách hàng vuốt ngón tay từ trên xuống hơn 40px thì sẽ đóng Popup
                if (touchEndY - touchStartY > 40) {
                  closeDrawer();
                }
              }}
            >
              {/* Thanh kéo nhỏ (Pull Indicator) */}
