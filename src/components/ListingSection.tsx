...
import { MapPin, Compass, Clock, Square, Bed, ChevronRight, ArrowLeft } from "lucide-react";
...
export default function ListingSection({ allBdsItems, forceDistrict }: ListingSectionProps) {
  ...
  // Quản lý sản phẩm được chọn hiển thị Popup
  const [selectedProduct, setSelectedProduct] = useState<RealEstateItem | null>(null);
  const [isNavigating, setIsNavigating] = useState(false);

  // Lưu và lấy vị trí trang từ sessionStorage
  const [currentPage, setCurrentPage] = useState(() => {
    if (typeof window !== "undefined") return parseInt(sessionStorage.getItem("bds_page") || "1");
    return 1;
  });
  const itemsPerPage = 6;

  // Tọa độ ngón tay phục vụ tính năng vuốt tay để back (Swipe to Back)
  const touchStartX = useRef<number>(0);
  const touchStartY = useRef<number>(0);

  // Khóa cuộn trang nền khi Popup đang mở
  useEffect(() => {
  ...
  // Cuộn mượt lên đầu danh sách khi bấm chuyển trang
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (parseInt(sessionStorage.getItem("bds_page") || "1") === 1) setCurrentPage(1);
  ...
  // Kích hoạt chuyển trang chi tiết chính thức và giữ nguyên vị trí phân trang cũ
  const handleViewDetail = (slug: string) => {
    setIsNavigating(true);
    router.push(`/nha-dat/${slug}`);
  };

  // Bắt đầu chạm tay (Tính năng Vuốt để quay lại)
  const handleTouchStart = (e: React.TouchEvent) => {
    touchStartX.current = e.touches[0].clientX;
    touchStartY.current = e.touches[0].clientY;
  };

  // Nhấc tay - Kiểm tra nếu vuốt từ cạnh trái sang phải thì đóng Popup
  const handleTouchEnd = (e: React.TouchEvent) => {
    const diffX = e.changedTouches[0].clientX - touchStartX.current;
    const diffY = e.changedTouches[0].clientY - touchStartY.current;
    if (touchStartX.current < 40 && diffX > 70 && Math.abs(diffY) < 40) {
  ...
  return (
    <>
      {/* 1. KHU VỰC THANH BỘ LỌC TÌM KIẾM */}
      ...
      {/* 2. HIỂN THỊ LƯỚI SẢN PHẨM SƠ BỘ */}
      <main id="listing-section" className="max-w-7xl mx-auto w-full px-4 mt-16 mb-20 scroll-mt-28">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-7">
          {currentItems.map((item) => (
            <div key={item.id} onClick={() => setSelectedProduct(item)} className="bg-white rounded-3xl overflow-hidden border border-slate-100 shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col group transform hover:-translate-y-1 cursor-pointer">
              <div className="relative aspect-[4/3] overflow-hidden bg-slate-100">
                <Image src={layUrlAnhChuan(item.anh)} alt={item.tieude} fill className="object-cover group-hover:scale-105 transition-transform duration-700" sizes="(max-w-7xl) 100vw" priority />
      ...
      {/* 3. GIAO DIỆN POPUP PHỦ KÍN TOÀN MÀN HÌNH + CẢM ỨNG VUỐT CHẠM GIỐNG APP */}
      {selectedProduct && (
        <div 
          onTouchStart={handleTouchStart}
          onTouchEnd={handleTouchEnd}
          className="fixed inset-0 bg-white z-[999] flex flex-col w-full h-screen overflow-y-auto animate-in fade-in slide-in-from-bottom duration-200"
        >
          {/* Header của Popup (Sticky) */}
          <div className="sticky top-0 bg-white border-b border-slate-100 px-4 py-4 flex items-center gap-3 z-20 shadow-sm">
            <button onClick={() => setSelectedProduct(null)} className="text-slate-800 hover:bg-slate-100 p-2 rounded-full transition-colors flex items-center justify-center">
          ...
          {/* Chi tiết nội dung sản phẩm bên trong Popup */}
          <div className="w-full max-w-4xl mx-auto flex-1 pb-24">
            <div className="relative w-full aspect-[16/10] sm:aspect-[21/9] bg-slate-100 sm:rounded-2xl sm:mt-4 overflow-hidden">
              <Image src={layUrlAnhChuan(selectedProduct.anh)} alt={selectedProduct.tieude} fill className="object-cover" sizes="(max-w-4xl) 100vw" priority />
              <span className={`absolute top-4 left-4 text-xs px-3 py-1.5 rounded-xl shadow-md ${getTagStyle(selectedProduct.tag)}`}>{selectedProduct.tag || 'Nhà Đất'}</span>
              <span className="absolute bottom-4 right-4 bg-slate-900/95 text-amber-400 font-black text-lg px-5 py-2 rounded-2xl shadow-xl">{selectedProduct.gia}</span>
            </div>

            <div className="p-4 sm:p-6 space-y-6">
              <div className="space-y-2">
                <div className="flex items-center gap-1.5 text-slate-500 text-xs font-bold uppercase"><MapPin className="w-4 h-4 text-amber-500 shrink-0" /><span>{selectedProduct.khuVucFull}</span></div>
                <h1 className="text-xl sm:text-2xl font-extrabold text-slate-900 leading-snug">{selectedProduct.tieude}</h1>
              </div>
              ...
              <div className="grid grid-cols-3 gap-3 bg-slate-50 p-5 rounded-2xl text-center border border-slate-100">
                <div className="border-r border-slate-200">
                  ...
                  <span className="text-base font-black text-slate-800 block mt-0.5">{selectedProduct.dienTich}</span>
                </div>
                <div className="border-r border-slate-200">
                  ...
                  <span className="text-base font-black text-slate-800 block mt-0.5">{selectedProduct.phongNgu || 'Đất ở'}</span>
                </div>
                <div>
                  <Compass className="w-6 h-6 text-amber-500 mx-auto mb-1" />
                  <span className="text-[11px] text-slate-400 font-medium uppercase tracking-wider">Hướng</span>
                  <span className="text-base font-black text-slate-800 block mt-0.5">{selectedProduct.huong || 'Chưa rõ'}</span>
                </div>
              </div>

              <div className="flex items-center gap-2 text-xs text-slate-400 border-t border-slate-100 pt-4 font-medium">
                <Clock className="w-4 h-4" /><span>Cập nhật: {formatTimeAgo(selectedProduct.ngayDang)} ({selectedProduct.ngayDang})</span>
              </div>
            </div>
          </div>

          {/* Thanh công cụ hành động dưới đáy Popup */}
          <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-100 p-4 shadow-[0_-4px_20px_rgba(0,0,0,0.05)] z-20 flex justify-center">
            <div className="w-full max-w-4xl flex gap-3">
              <button onClick={() => setSelectedProduct(null)} className="flex-1 bg-slate-100 text-slate-700 text-sm font-bold py-4 rounded-xl hover:bg-slate-200 transition-colors">Quay lại danh sách</button>
              <button onClick={() => handleViewDetail(selectedProduct.slug)} className="flex-1 bg-amber-500 text-slate-900 text-sm font-bold py-4 rounded-xl hover:bg-amber-400 transition-colors text-center flex items-center justify-center gap-1 shadow-md">
