...
export default function ListingSection({ allBdsItems, forceDistrict }: ListingSectionProps) {
  ...
  useEffect(() => {
    ...
    if (selectedTag === "mattien") {
      result = result.filter(i => i.isMatTien === true);
    } else if (selectedTag === "chinhchu") {
      result = result.filter(i => i.tag?.includes("Chính Chủ"));
    }
  ...
  // Thuật toán chuyển đổi ngày tháng tính khoảng thời gian đăng tin thực tế
  const formatTimeAgo = (dateStr: string) => {
    ...
    const ngayDang = new Date(year, month, day);
    const homNay = new Date();
    ngayDang.setHours(0,0,0,0); homNay.setHours(0,0,0,0);
    
    const diffTime = homNay.getTime() - ngayDang.getTime();
    ...
    <>
        ...
        <div class="bg-white p-5 sm:p-6 rounded-3xl border border-slate-100 shadow-xl space-y-4">
          <div class="grid grid-cols-2 gap-3 md:grid-cols-4 md:gap-4">
            <div>
              ...
              <select disabled={!!forceDistrict} value={khuVuc} onChange={(e) => setKhuVuc(e.target.value)} class="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-3 text-sm font-semibold focus:outline-none focus:border-amber-500 cursor-pointer text-slate-700">
            ...
            <div>
              ...
              <select value={loaiHinh} onChange={(e) => setLoaiHinh(e.target.value)} class="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-3 text-sm font-semibold focus:outline-none focus:border-amber-500 cursor-pointer text-slate-700">
            ...
            <div>
              ...
              <select value={khoangGia} onChange={(e) => setKhoangGia(e.target.value)} class="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-3 text-sm font-semibold focus:outline-none focus:border-amber-500 cursor-pointer text-slate-700">
            ...
            <div>
              ...
              <select value={huong} onChange={(e) => setHuong(e.target.value)} class="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-3 text-sm font-semibold focus:outline-none focus:border-amber-500 cursor-pointer text-slate-700">
          ...
          <div class="flex flex-wrap gap-2 pt-3 border-t border-slate-100 items-center">
            ...
            <button onClick={() => setSelectedTag("all")} class={`text-xs font-bold px-4 py-2 rounded-xl transition-all ${selectedTag === "all" ? "bg-slate-900 text-white shadow-sm" : "bg-white border border-slate-200 text-slate-600"}`}>Tất Cả</button>
            <button onClick={() => setSelectedTag("mattien")} class={`text-xs font-bold px-4 py-2 rounded-xl transition-all ${selectedTag === "mattien" ? "bg-slate-900 text-white shadow-sm" : "bg-white border border-slate-200 text-slate-600"}`}>Mặt Tiền Kinh Doanh</button>
            <button onClick={() => setSelectedTag("chinhchu")} class={`text-xs font-bold px-4 py-2 rounded-xl transition-all ${selectedTag === "chinhchu" ? "bg-slate-900 text-white shadow-sm" : "bg-white border border-slate-200 text-slate-600"}`}>Hàng Chính Chủ</button>
          </div>
        </div>
      </section>
      ...
      <main id="listing-section" class="max-w-7xl mx-auto w-full px-4 mt-16 mb-20">
        <div class="flex flex-col md:flex-row md:items-end justify-between mb-8 gap-3">
          ...
          <p class="text-sm text-slate-400 font-medium">Hình ảnh khảo sát thực tế • Không tin ảo</p>
        </div>
            ...
            return (
              <article key={item.id} onClick={() => { setSelectedItem(item); setIsOpenDetail(true); }} class="bg-white rounded-3xl overflow-hidden border border-slate-100 shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col group cursor-pointer transform hover:-translate-y-1">
                <div class="relative aspect-[4/3] overflow-hidden bg-slate-100">
                  ...
                  {item.huong && <span class="absolute top-3 right-3 bg-white/95 backdrop-blur-sm text-slate-800 font-extrabold text-[10px] px-2.5 py-1 rounded-lg shadow-sm flex items-center gap-1"><Compass class="w-3 h-3 text-amber-500" />{item.huong}</span>}
                  <span class="absolute bottom-3 left-3 bg-black/60 backdrop-blur-sm text-white text-[10px] font-semibold px-2.5 py-1 rounded-lg flex items-center gap-1">
                    <Clock class="w-3 h-3 text-amber-400" /> {formatTimeAgo(item.ngayDang)}
                  </span>
                ...
                <div class="p-5 flex-1 flex flex-col justify-between">
                  ...
                  <div class="mt-4 pt-4 border-t border-slate-50 flex items-center justify-between text-slate-500 text-sm font-medium">
                    ...
                    <span class="text-amber-500 font-bold flex items-center gap-0.5 text-xs uppercase tracking-wider group-hover:translate-x-0.5 transition-transform">Chi tiết <ChevronRight class="w-3 h-3" /></span>
                  </div>
                </div>
              </article>
            );
        ...
        {totalPages > 1 && (
          <div class="flex justify-center items-center gap-2 mt-10">
            {Array.from({ length: totalPages }, (_, index) => (
              <button key={index} onClick={() => setCurrentPage(index + 1)} class={`w-9 h-9 rounded-xl text-sm transition-all active:scale-95 font-bold ${currentPage === index + 1 ? "bg-amber-500 text-slate-900 shadow-sm font-extrabold scale-105" : "bg-white border border-slate-200 text-slate-600 hover:border-slate-300"}`}>
                {index + 1}
              </button>
            ))}
          </div>
      ...
      {selectedItem && (
        <Modals type="detail" isOpen={isOpenDetail} item={selectedItem} onClose={() => { setIsOpenDetail(false); setSelectedItem(null); }} />
      )}
    </>
  );
}
