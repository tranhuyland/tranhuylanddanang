...
// ==========================================
// 5. COMPONENT THẺ BĐS (Đã Cập Nhật 3 Hiệu Ứng)
// ==========================================
function BdsCard({ item }: { item: any }) {
  ...
  return (
    {/* HIỆU ỨNG THẺ LÚN XUỐNG: Sửa hover:-translate-y-1.5 thành hover:translate-y-1.5, thêm active:translate-y-2 */}
    <a href={`/nha-dat/${item.slug}`} className="group bg-white rounded-3xl overflow-hidden shadow-sm hover:shadow-md border border-slate-100 hover:border-orange-300 transition-all duration-300 flex flex-col h-full relative transform hover:translate-y-1.5 active:translate-y-2 block cursor-pointer">
      <div className="relative aspect-[16/10] w-full overflow-hidden bg-slate-100">
        {/* HIỆU ỨNG ẢNH PHÓNG TO: Đã có sẵn group-hover:scale-110, hoạt động hoàn hảo */}
      ...
      <div className="p-5 flex flex-col flex-grow justify-between">
        <div>
          ...
          {/* HIỆU ỨNG TIÊU ĐỀ CAM: Chuyển group-hover:text-orange-600 thành group-hover:text-orange-500 */}
          <h3 className="text-slate-800 font-extrabold text-[15px] sm:text-base line-clamp-2 group-hover:text-orange-500 transition-colors duration-300 h-[2.6rem] sm:h-[3rem] overflow-hidden leading-snug mb-4">
            {item.tieude}
          </h3>
        </div>
