    ...
    <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white w-full max-w-sm rounded-2xl overflow-hidden shadow-2xl p-6 relative">
        ...
        <p className="text-xs text-slate-400 mb-4">
          Thông tin đăng ký sẽ tự động chuyển tiếp sang ứng dụng Zalo anh Trần Huy.
        </p>
        <form onSubmit={handleKyGuiSubmit} className="space-y-3 text-sm">
          <div>
            ...
            <input 
              ...
              className="w-full bg-slate-50 border rounded-xl px-3 py-2.5 focus:outline-none focus:border-amber-500" 
            />
          </div>
          <div>
            <label className="block font-bold text-slate-600 mb-1">Địa Chỉ Ký Gửi *</label>
            <input 
              type="text" 
              required 
              value={kgDiaChi} 
          ...
          <button 
            type="submit" 
            className="w-full bg-slate-900 text-white font-bold rounded-xl py-3 text-sm mt-3 shadow-md transition-all active:scale-95"
          >
            Xác Nhận Ký Gửi
          </button>
        </form>
